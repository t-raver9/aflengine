import pandas as pd
from metadata import home_grounds
...
def home_ground_advantage(matches: pd.DataFrame) -> pd.DataFrame:
    # Initialise columns
    matches['h_ground_advantage'] = 0
    matches['a_ground_advantage'] = 0

    # Determine if the home and away teams have a home-ground advantage
    for idx,row in matches.iterrows():
        if row['venue'] in home_grounds[row['hteam']]:
            matches.at[idx,'h_ground_advantage'] = 1
        if row['venue'] in home_grounds[row['ateam']]:
            matches.at[idx,'a_ground_advantage'] = 1
